...
#        args_dict = OrderedDict(list(itertools.izip(args_name, args)) + list(kwargs.iteritems()))
        args_dict = OrderedDict(list(kwargs.iteritems()))
        global params
        params = args_dict
#        print args_dict
...
def func2(window_back,window_back_min_num,sr):
...
#                            ,min_periods = window_back_min_num + 1).apply(lambda x:get_signal(x))
    ...
    col_name = df.columns
    func_ = partial(func2,window_back,window_back_min_num)
    sr_list = Parallel(n_jobs=16, verbose=2)( delayed(func_)(col[1]) for col in df.iteritems())
    ...
    #        pdb.set_trace()
            ...
            sr_list = Parallel(n_jobs=32, verbose=2)( delayed(func)(col[1]) for col in df_signal.iteritems())
            df_signal_rank = pd.concat(sr_list,axis=1)
            df_signal_rank = df_signal_rank[col_name]
#            df_signal_rank = df_signal_rank.rank(method = 'average'                                      # 相同值取排位均值
#                                                ,na_option ='keep'                                      # 對nan保留
#                                                ,pct=True                                               # 計算百分比，從0-1
#                                                ,axis = rank_axis
#                                                )
...
@timeit
def plot_top_return(df_return,df_price):
...
#    fail_dates = df_return.sum(1).sort_values(ascending=False).tail(Days).index
    ...
    for i,dic in enumerate(all_dic.iteritems()):
        date = dic[0]
        codes = dic[1][0]
...
#    ax1.fill_between(df_return.sum(1).cumsum().index,0,df_return.sum(1).cumsum(), alpha=0.3)
    ...
    plot_top_return(df_return,df_Panel['close'])
#    fig.autofmt_xdate()
